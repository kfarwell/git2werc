...
# git2werc - Convert a git repository to a set of static HTML pages
# suitable for werc.
# Copyright (c) 2016 Kyle Farwell <m@kfarwell.org>
# Based on git2html Copyright (c) 2011 Neal H. Walfield
# <neal@walfield.org>
#
...
html_footer()
{
  echo "<hr>" \
    "Generated by" \
    "<a href=\"http://kfarwell.org/projects/git2werc\">git2werc</a>."
}
...
#  fi
  ...
  then
    echo  "<p>Clone this repository using:</p>" \
      "<pre>" \
      " git clone $PUBLIC_REPOSITORY" \
      "</pre>"
  fi

  echo "<h2>Contributing</h2>" \
    "<p>E-mail patches generated with <code>git format-patch</code> to kfarwell@member.fsf.org</b>.</p>"
...
do
  ...
  do
    ...
    echo "<tr><td valign=\"middle\"><pre>$graph</pre></td><td><a href=\"../commits/$commit/\">$log</a></td><td>$author</td><td><i>$date</i></td></tr>" \
	>> "$BRANCH_INDEX"
    ...
    COMMIT_INDEX="$COMMIT_BASE/index.html"
    {
      ...
      # The metadata.
      echo "<h2>Branch: <a href=\"../../branches/$branch\">$branch</a></h2>" \
	"<p>Author: $author" \
	"<br>Date: $date" \
        "<br>Commit: $commit"
      for p in $parent
	...
	" (<a href=\"../../commits/$commit/diff-to-$p\">diff to parent</a>)"
      done
      echo "<br>Log message:" \
	"<p><pre>$loglong</pre>"
      for p in $parent
      do
	echo "<p>Diff Stat to $p:</p>" \
	  "<blockquote><pre>"
        git diff --stat $p..$commit \
          | gawk \
              '{ if (last_line) print last_line;
                 last_line_raw=$0;
                 $1=sprintf("<a href=\"%s.raw\">%s</a>" \
                            " (<a href=\"../../commits/'"$p"'/%s.raw\">old</a>)" \
                            "%*s" \
                            "(<a href=\"diff-to-'"$p"'#%s\">diff</a>)",
                            $1, $1, $1, 60 - length ($1), " ", $1);
                    last_line=$0; }
	  ...
	  | gawk '
           ...
           }
           {
             ...
             printf ("<li><a name=\"files:%s\" href=\"%s.raw\">%s</a>\n",
                     $0, $0, file);
    ...
    do
      {
        ...
        echo "<h2>Branch: <a href=\"../../branches/$branch\">$branch</a></h2>" \
          "<h3>Commit: <a href=\"index\">$commit</a></h3>" \
  	"<p>Author: $author" \
  	"<br>Date: $date" \
